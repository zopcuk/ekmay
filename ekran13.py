...
def lockButton():
    ...
    textLabel = tk.Label(entryLabel,bg=labelColor,
                         font="Helvetica 20 bold",foreground="black", highlightthickness=0, border=0)
    textLabel.grid(column=0, row=0,sticky="ewns")
    e = tk.Entry(entryLabel, textvariable=password, show='*', justify='center', width=10, font="Helvetica 44 bold",
                 bg=labelColor, foreground="black", highlightthickness=0, border=0)
    e.grid(column=1, row=0, sticky="nsew")

    b_pc = tk.Button(entryLabel, image=password_change, bg=labelColor, border=0, highlightthickness=0,
                            activebackground=labelColor)
    b_pc.grid(column=2, row=0, sticky="ewns")

    def pcRelease(event):
        button_list = [b1, b2,b3,b4,b5]
        textLabel.configure(text="Enter\nNew\nPassword")
        b_pc.configure(image=cancel)
        for i in range(4):
            button_list[i].configure(bg="gray")
    b_pc.bind("<ButtonRelease>", pcRelease)
